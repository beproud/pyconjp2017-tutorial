...
`find_all()` 関数の第1引数のname引数を指定すると取得するタグを制限する事ができます。
...
今度は `h2` タグ と `article-title` 属性を指定してHTMLの記事の要素を取得する様に修正を行い、要素から記事のタイトル名とリンクのURLのみを出力するようにしてみました。
...
.. note::
  ...
  予約後とは?: プログラミング言語において字句的には識別子（変数名、関数名など）としてのルールを満たしているにもかかわらず、識別子として使えない字句要素です。

  Pythonでは以下のコードで予約後の一覧を確認する事ができます。
